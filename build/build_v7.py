...
<div class="chartcard fade"><svg id="chart" viewBox="0 0 1000 772" preserveAspectRatio="xMidYMid meet"></svg>
<div class="legend">
...
// ---- CVD pane (Heikin-Ashi cumulative volume delta + 50 EMA on HA close) ----
const vT=600,vB=730;
svg.appendChild(el('line',{{x1:padL,y1:vT-8,x2:padL+plotW,y2:vT-8,stroke:'#1a2330','stroke-width':1}}));
txt(padL,vT-12,'CVD  (Heikin-Ashi cumulative delta · 50 EMA blue) — below = bearish bias',{{fill:'#7d8da3','font-size':9.5}});
const vv=bars.flatMap(b=>[b.cvh,b.cvl,b.cve50]);const vMin=Math.min(...vv),vMax=Math.max(...vv);const vpad=(vMax-vMin)*0.08||1;
const vy=v=>vT+(vMax+vpad-v)/((vMax+vpad)-(vMin-vpad))*(vB-vT);
const cvw=Math.max(2,plotW/bars.length*0.58);
bars.forEach((b,i)=>{{const up=b.cvup===1;const col=up?'#39ff88':'#e5484d';
 svg.appendChild(el('line',{{x1:x(i),y1:vy(b.cvh),x2:x(i),y2:vy(b.cvl),stroke:up?'#74e05e':'#ffad7d','stroke-width':1}}));
 const yo=vy(b.cvo),yc=vy(b.cvc);
 svg.appendChild(el('rect',{{x:x(i)-cvw/2,y:Math.min(yo,yc),width:cvw,height:Math.max(1.5,Math.abs(yc-yo)),fill:col,stroke:up?'#74e05e':'#ffad7d','stroke-width':0.5}}));
}});
// 50 EMA on HA close — blue dashed medium (same spec as the 144)
let ce='';bars.forEach((b,i)=>{{ce+=(i?'L':'M')+x(i)+','+vy(b.cve50)+' ';}});
svg.appendChild(el('path',{{d:ce,fill:'none',stroke:'#4d9fff','stroke-width':2.4,'stroke-dasharray':'7 4'}}));
txt(W-padR-4,vy(bars[bars.length-1].cve50)+4,'50 EMA',{{fill:'#4d9fff','font-size':9,'text-anchor':'end','font-weight':600}});
// time axis along the bottom (like TV) — every 6th bar
bars.forEach((b,i)=>{{ if(i%6===0 || i===bars.length-1){{
 svg.appendChild(el('line',{{x1:x(i),y1:vB+3,x2:x(i),y2:vB+7,stroke:'#3a4658','stroke-width':1}}));
 txt(x(i),vB+18,b.et,{{fill:'#7d8da3','font-size':9,'text-anchor':'middle'}});
}} }});
...
open('/home/claude/trade_lesson_june4_v7_cvd.html','w').write(html)
