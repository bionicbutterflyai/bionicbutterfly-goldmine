...
# ===== CVD (LonesomeTheBlue) -> Heikin-Ashi candles + 50 EMA on HA close =====
opn=[r[1] for r in rows]; hi=[r[2] for r in rows]; lo=[r[3] for r in rows]
def _rate(i,cond):
    tw=hi[i]-max(opn[i],close[i]); bw=min(opn[i],close[i])-lo[i]; body=abs(close[i]-opn[i])
    den=tw+bw+body
    if den==0: return 0.5
    r=0.5*(tw+bw+(2*body if cond else 0))/den
    return 0.5 if r==0 else r
cum=[]; acc=0.0
for i in range(n):
    du=vol[i]*_rate(i, opn[i]<=close[i]); dd=vol[i]*_rate(i, opn[i]>close[i])
    acc += du if close[i]>=opn[i] else -dd
    cum.append(acc)
# candle OHLC from cumdelta, then Heikin-Ashi
chac=[0.0]*n; chao=[0.0]*n; chah=[0.0]*n; chal=[0.0]*n
for i in range(n):
    cprev=cum[i-1] if i>0 else cum[i]
    o_=cprev; h_=max(cum[i],cprev); l_=min(cum[i],cprev); c_=cum[i]
    chac[i]=(o_+h_+l_+c_)/4
    chao[i]=(o_+c_)/2 if i==0 else (chao[i-1]+chac[i-1])/2
    chah[i]=max(h_,chao[i],chac[i]); chal[i]=min(l_,chao[i],chac[i])
cvd_e50=ema_series(chac,50)
...
for i,r in enumerate(rows):
    ...
    if '1418'<=hhmm<='1522':
        ...
        out.append({'et':et,'o':r[1],'h':r[2],'l':r[3],'c':r[4],'v':int(r[5]),
                    ...
                    'st':barstate(i),
                    'cvo':round(chao[i],1),'cvh':round(chah[i],1),'cvl':round(chal[i],1),
                    'cvc':round(chac[i],1),'cvup':1 if chao[i]<=chac[i] else 0,
                    'cve50':round(cvd_e50[i],1)})
