...
sp=[b['sma'] for b in bars if b['et'] in ('10:57','10:58','10:59') and b['sma']]
...
<div class="leg gate"><div class="k">3-green gate</div><div class="v">✓ aligned 10:58</div><div class="s">candle+cloud+DPMO</div></div>
...
const W=1000,H=580,padL=8,padR=80;
// price pane
...
// EMA 20/50 cloud — per-bar fill (green when 20>=50, red when 50>20), like TV
let p20='',p50='';bars.forEach((b,i)=>{{p20+=(i?'L':'M')+x(i)+','+y(b.e20)+' ';p50+=(i?'L':'M')+x(i)+','+y(b.e50)+' ';}});
for(let i=0;i<bars.length-1;i++){{const a=bars[i],b=bars[i+1];const up=((a.e20-a.e50)+(b.e20-b.e50))/2>=0;
 ...
 svg.appendChild(el('polygon',{{points:poly,fill:up?'#2ecc8f':'#e5484d','fill-opacity':0.12,stroke:'none'}}));}}
svg.appendChild(el('path',{{d:p20,fill:'none',stroke:'#2ecc8f','stroke-width':1.1,'stroke-opacity':0.5}}));
svg.appendChild(el('path',{{d:p50,fill:'none',stroke:'#e5a73c','stroke-width':1.1,'stroke-opacity':0.5}}));
txt(x(bars.length-1)-2,y(bars[bars.length-1].e20)-4,'20/50',{{fill:'#7d8da3','font-size':9,'text-anchor':'end'}});
// exit/stop zones
const ix=x(idxOf('10:57'));
...
txt(W-padR-4,y(EXIT)-5,'EXIT '+EXIT+' · GEX',{{fill:'#e5484d','font-size':10,'text-anchor':'end','font-weight':600}});
svg.appendChild(el('line',{{x1:ix,y1:y(SL),x2:padL+plotW,y2:y(SL),stroke:'#e5484d','stroke-width':1.1,'stroke-dasharray':'5 4'}}));
txt(W-padR-4,y(SL)+12,'SL '+SL+' (40t)',{{fill:'#e5484d','font-size':10,'text-anchor':'end'}});
// 144 spine
...
txt(W-padR-4,y([...bars].reverse().find(b=>b.sma).sma)+4,'144',{{fill:'#4d9fff','font-size':10,'text-anchor':'end','font-weight':600}});
...
(function(){{const i=idxOf('10:57'),px=x(i),yTop=y(ENTRY),yBot=y(bars[i].sma),bx=padL+8;
...
mk('10:24',bars[idxOf('10:24')].l,'#d4a017','down','ALERT');
mk('10:49',bars[idxOf('10:49')].l,'#9b6dff','down','PULLBACK');
mk('10:58',ENTRY,'#39ff88','down','ENTRY');
mk('11:15',EXIT,'#e5484d','up','EXIT');
// ---- oscillator pane ----
...
const cx=x(idxOf('10:58'));svg.appendChild(el('line',{{x1:cx,y1:oT,x2:cx,y2:oB,stroke:'#39ff88','stroke-width':1,'stroke-dasharray':'2 3','stroke-opacity':0.7}}));
txt(cx+3,oB-4,'cloud→green 10:58',{{fill:'#39ff88','font-size':8.5}});
...
document.getElementById('lessonText').innerHTML=
 `Morning sold to ~30152, then the reclaim set up. <span class="hl">Alert</span> ~10:24 as price held back over the 144; <span class="hl">pullback</span> into ~30287 by 10:49; then the gate completed — <em>candle, the 20/50 cloud, and the DPMO cloud all green</em>. `+
 `Watch the lower pane: the DPMO cloud flips <em>green at 10:58</em>, and one minute later the <span class="vib">11:01 bar prints vibrant green on ~11.8k volume</span> (your high-volume tell). Price ran to the heavy <em>GEX level at 30451</em>, where you exited ahead of the rollover — <em>+101 points</em> available.`;
document.getElementById('gapNote').innerHTML=
 `<b>⚠ One bar to reconcile:</b> my port of your DPMO turns the cloud <b>green at 10:58</b> — the first bar all three confirmations align — but your stated entry was <b>10:57</b>, while the DPMO was still bearish (<code>d ≈ −0.000</code>, right at the cross). Did you enter on the 10:57 cross <i>forming</i>, or does your live DPMO flip a bar earlier (seeding/settings)? `+
 `Also: the engine still flags <b>no fresh 144 break</b> (~{ABOVE:.0f} pts above the spine) — a <b>false negative</b> vs the 3-green gate. Both are cues to teach the engine.`;
...
open('/home/claude/trade_lesson_june4_v4.html','w').write(html)
