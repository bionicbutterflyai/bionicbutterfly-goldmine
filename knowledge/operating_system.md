...
- **Bank discipline.** Bank on Mark's explicit GO only, as ONE full-repo zip → replace-all (never batches,
  never keep-both). Propagate every dependent VIEW the same session (the Propagation Law).
